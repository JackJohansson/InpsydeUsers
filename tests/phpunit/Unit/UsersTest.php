<?php
	...
	class UsersTest extends TestCase {
		...
		/**
		 * Test the flush_all() method. This method only returns
		 * true, or a WP_Error.
		 *
		 */
		public function testFlush_all() {
			$users   = new \Inpsyde\Users();
			$results = $users->flush_all();
		...
		/**
		 * Test the flush_user() method. This method only
		...
		/**
		 * Test the get_results method. This method only returns
		...
		public function testGet_results() {
			...
			$users   = new \Inpsyde\Users();
			$results = $users->get_results( \Inpsyde\Restful::REST_USERS_ROUTE );
		...
		/**
		 * Test the list_users() method. This method will only return
		 * a non-empty array, or an instance of WP_Error.
		 *
		 */
		public function testList_users() {
		...
		/**
		 * Test the results of the make_request() method.
		...
		public function testMake_Request() {
			...
			$results = $users->make_request( \Inpsyde\Restful::REST_USERS_ROUTE );
		...
		/**
		 * Test the results of user_info() method. This method
		 * returns an array of data on success, or a WP_Error
		 * on failure.
		 *
		 */
		public function testUser_info() {
			$users = new \Inpsyde\Users();

			// This should always be either an array or a WP_Error.
			$results = $users->user_info( 0 );
			// Assert that there's no error.
